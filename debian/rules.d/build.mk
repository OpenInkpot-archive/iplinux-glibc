...
ifeq ($(DEB_CROSS_BOOTSTRAP),second)
...
$(stamp)configure_%: $(stamp)mkbuilddir_%
	...
	echo "CFLAGS = $(BUILD_CFLAGS)"	>> $(DEB_BUILDDIR)/configparms
	...
	echo "configure_build=$$configure_build"; \
	if [ $(call xx,configure_target) = $$configure_build ]; then \
	  ...
	  if  $(call kernel_check,$(call xx,MIN_KERNEL_SUPPORTED)); then \
	    configure_build=`echo $$configure_build | sed -e 's/^\([^-]*\)-\([^-]*\)$$/\1-dummy-\2/' -e 's/^\([^-]*\)-linux-\([^-]*\)$$/\1-dummy-\2/'`; \
	...
	$(call logme, -a $(log_build), echo -n "extra flags: $(call xx,extra_config_options) ") ; \
	$(call logme, -a $(log_build), echo -n "addons: $(standard-add-ons) $(call xx,add-ons) ") ; \
...
$(stamp)build_%: $(stamp)configure_%
	...
	if [ $(curpass) = libc ]; then \
		unset ARCH CC LD CFLAGS; \
		$(call logme, -a $(log_build), $(MAKE) -C $(DEB_BUILDDIR) -j $(NJOBS) lib); \
...
$(stamp)install_%: $(stamp)check_%
	...
	if [ $(curpass) = libc ]; then \
		$(MAKE) -C $(DEB_BUILDDIR) -j $(NJOBS) \
		  ...
		  libdl=`pwd`/$(DEB_BUILDDIR)/dlfcn/libdl.so \
		  $(install_target); \
	fi
	

	# Well...
	# We cannot compile binaries during 2nd stage SO
	# 1. Either we do 3rd stage
	# 2. Or simply forget about them
